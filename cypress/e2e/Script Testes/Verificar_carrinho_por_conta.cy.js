describe('Teste de login, carrinho e validação de usuários no Demoblaze', () => {
    ...
    //   });
      ...
      cy.get('.name').invoke('text').then((itemName) => {
        ...
        cy.log(`Item capturado: ${itemUserA}`);
    });
    ...
    cy.get('.success td:nth-child(2)').then(($el) => {
        ...
        expect(cartItemName).to.contain(itemUserA); // Valida que o item no carrinho corresponde
    });
    
    cy.log('Item confirmado no carrinho');
    ...
    //    });
       ...
       cy.get('.success td:nth-child(2)').then(($el) => {
           const cartItemName = $el.text().trim();
           cy.log(`Nome do item no carrinho: ${cartItemName}`);
           expect(cartItemName).to.contain(itemUserB); // Valida que o item no carrinho corresponde
       });
